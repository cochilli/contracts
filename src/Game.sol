...
contract CochilliGameBeta is AccessControl, Pausable {
    ...
    uint256 public maxBet = 50e6;
    uint256 public maxUtilizedLiquidity = 255e6;
    uint24 public cancelBuffer = 1 days;
    uint24 public maxInterval = 1 days;
    uint16 public minInterval = 2 minutes;
    ...
    event UpdatedIntervals(uint16 minInterval, uint24 maxInterval, uint24 cancelBuffer);
    ...
    function placeBet(
        ...
        emit BetPlaced(betId, msg.sender, pair, block.timestamp, block.timestamp + interval, interval);
    }
    ...
    function cancelBet(uint256 betId) external {
        ...
        if (closeBetDetails.endTime + cancelBuffer >= block.timestamp) revert InvalidTimestamp(); 
        if (!closeBetDetails.active) revert InactiveBet();

        uint256 amount = closeBetDetails.amount;
        betIds[betId].active = false;
        lockedLiquidity -= closeBetDetails.payout;
    ...
    function setTimes(uint16 _minInterval, uint24 _maxInterval, uint24 _cancelBuffer) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_minInterval == 0 || _maxInterval == 0 || _minInterval >= _maxInterval) revert InvalidInterval();
        minInterval = _minInterval;
        maxInterval = _maxInterval;
        cancelBuffer = _cancelBuffer;

        emit UpdatedIntervals(minInterval, maxInterval, cancelBuffer);
    }
